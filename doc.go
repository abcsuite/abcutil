...
/*
Package abcutil provides Aero-specific convenience functions and types.

Block Overview

A Block defines a Aero block that provides easier and more efficient
...
A Tx defines a Aero transaction that provides more efficient manipulation of
...
The Address interface provides an abstraction for a Aero address.  While the
most common type is a pay-to-pubkey-hash, Aero already supports others and
...
To decode/encode an address:
	...
	addr, err := abcutil.DecodeAddress(addrString, defaultNet)
	if err != nil {
		fmt.Println(err)
		return
	}
