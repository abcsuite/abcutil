...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := abcutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(abcutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := abcutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := abcutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
