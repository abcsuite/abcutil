...
// Tx returns a wrapped transaction (abcutil.Tx) for the transaction at the
...
// STx returns a wrapped transaction (abcutil.Tx) for the stake transaction at
...
// Transactions returns a slice of wrapped transactions (abcutil.Tx) for all
...
// instead provides easy access to wrapped versions (abcutil.Tx) of them.
...
// STransactions returns a slice of wrapped stake transactions (abcutil.Tx) for all
// stake transactions in the Block.  This is nearly equivalent to accessing the raw
// transactions (ABCwire.MsgTx) in the underlying wire.MsgBlock, however it
