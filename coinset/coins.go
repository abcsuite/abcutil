...
func (cs *CoinSet) TotalValue() (value abcutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue abcutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue abcutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount abcutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue abcutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue abcutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue abcutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue abcutil.Amount, coins []Coin) (Coins, error) {
...
func (c *SimpleCoin) Value() abcutil.Amount {
	return abcutil.Amount(c.txOut().Value)
}
